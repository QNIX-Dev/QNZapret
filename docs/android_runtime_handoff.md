...
- Telegram Cloudflare route layer строит `kwsN.<cfDomain>` и `kwsN-1.<cfDomain>` для каждого base domain, предпочитает `kwsN-1` для media/negative DC, сохраняет последний успешный CF domain как active, ставит HTTP 429 в cooldown на 45 секунд и пишет `telegram cf route start/attempt/ok/failed`.
...
- Для CF route selection добавлен in-memory EWMA score по DNS/TCP/TLS/WS handshake, first upstream payload, throughput и failures. Active domain остается первым внутри своего source bucket, плохие route не удаляются навсегда и могут восстановиться после cooldown/успешных сессий.
- Telegram compatibility proxy логирует session id, `dc/rawDc/mediaDc`, выбранный route, pooled hit/miss, DNS/TCP/TLS/WS timings, first client/ws payload, bytesUp/bytesDown, 5-секундный throughput window и close reason.
