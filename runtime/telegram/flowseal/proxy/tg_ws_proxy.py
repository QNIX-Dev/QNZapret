...
def _write_health(state: str) -> None:
    if not health_file:
        return
    directory = os.path.dirname(health_file)
    if directory:
...
async def _run(stop_event: Optional[asyncio.Event] = None):
    ...
    log.info("=" * 60)
    log.info("  Connect URI is available only through session D-Bus")
    log.info("=" * 60)
...
def main():
    ...
    ap.add_argument('--secret-file', type=str, default=None,
                    help='Read MTProto secret from a mode-0600 file.')
    ap.add_argument('--health-file', type=str, default=None,
                    help='Write listener/bridge readiness to this file.')
    ...
    if args.secret_file:
        try:
            with open(args.secret_file, 'r', encoding='ascii') as source:
                secret_hex = source.read().strip()
        except OSError as error:
            log.error("Unable to read secret file: %s",
